...
BEGIN
  ...
  -- content_unlocks.format is text, compare directly
  SELECT EXISTS(
  ...
  -- user_purchases.format is text, compare directly
  ...
  -- order_items.format is book_format_type ENUM — cast p_format to enum for proper comparison
  SELECT EXISTS(
...
BEGIN
  ...
  -- content_unlocks.format is text
  ...
  -- user_purchases.format is text
  IF EXISTS (SELECT 1 FROM public.user_purchases WHERE user_id = p_user_id AND book_id = p_book_id AND format = p_format AND status = 'active') THEN
    RETURN json_build_object('granted', true, 'method', 'purchase');
  END IF;

  -- order_items.format is book_format_type ENUM — cast p_format to enum
