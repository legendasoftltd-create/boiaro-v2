...
BEGIN
  ...
  SELECT EXISTS(
    ...
    WHERE oi.book_id = p_book_id AND oi.format::text = p_format
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM public.order_items oi JOIN public.orders o ON o.id = oi.order_id WHERE oi.book_id = p_book_id AND oi.format::text = p_format AND o.user_id = p_user_id AND o.status IN ('paid','confirmed','access_granted','completed','delivered')) THEN
    RETURN json_build_object('granted', true, 'method', 'purchase');
  END IF;
