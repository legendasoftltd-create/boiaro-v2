...
export default function EbookReader() {
  ...
  // Restore lock from sessionStorage on mount / bookId change
  useEffect(() => {
    if (isFreeBook && bookId && sessionStorage.getItem(`app_prompt_locked_ebook_${bookId}`)) {
      setContentLocked(true);
    }
  }, [bookId, isFreeBook]);

  const handleDownloadClick = () => {
    if (bookId) sessionStorage.setItem(`app_prompt_locked_ebook_${bookId}`, "1");
    setContentLocked(true);
  ...
  return (
    ...
    >
      ...
      {/* Locked overlay — covers the reader after user clicks download */}
      {contentLocked && (
        <div className="fixed inset-0 z-50 bg-background/95 backdrop-blur-md flex flex-col items-center justify-center p-8 text-center gap-4">
          ...
          <div>
            <h3 className="text-xl font-bold text-foreground mb-1">{brandName} অ্যাপে পড়ুন</h3>
            <p className="text-sm text-muted-foreground max-w-xs">
              সম্পূর্ণ বইটি বিনামূল্যে পড়তে অ্যাপ ডাউনলোড করুন।
            </p>
          </div>
