...
import { Music, Trash2, Loader2, Disc3 } from "lucide-react"
...
export default function AdminStudioLibrary() {
  ...
  const { data: assets = [], isLoading } = trpc.studio.libraryList.useQuery({ category: filter === "all" ? undefined : filter })
          ...
          {isLoading ? (
            ...
            platformAssets.map((a: any) => (
              <div key={a.id} className="flex items-center gap-3 p-2.5 rounded-lg bg-muted/30 text-sm">
                ...
                <audio controls src={a.file_url} className="h-8 max-w-[220px]" preload="none" />
                <Button
                  size="icon"
                  variant="ghost"
