...
export default function AdminRoleApplications() {
  ...
  const load = async () => {
    ...
    const { data } = await supabase
      ...
      .select("*, profiles!role_applications_user_id_fkey(display_name)")
      .eq("status", filter)
  ...
  const getName = (app: Application) => app.display_name || app.full_name || app.profiles?.display_name || "Unknown User";
                  ...
                  <div className="flex items-center gap-3 min-w-0 flex-1">
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        <Badge variant="outline" className={`text-[10px] capitalize ${ROLE_COLORS[app.requested_role] || ""}`}>
                          {app.requested_role}
                        </Badge>
                      ...
                      <p className="text-xs text-muted-foreground mt-0.5">
                        {app.email || "No email"} · {new Date(app.created_at).toLocaleDateString()}
                      </p>
                    </div>
                  </div>
            ...
            <div className="space-y-6">
              ...
              <div className="flex gap-4 items-start">
                ...
                <div className="flex-1">
                  ...
                  {selected.full_name && selected.display_name && selected.full_name !== selected.display_name && (
                    <p className="text-sm text-muted-foreground">Full name: {selected.full_name}</p>
                  )}
                  <div className="flex gap-2 mt-1.5">
                    <Badge variant="outline" className={`text-xs capitalize ${ROLE_COLORS[selected.requested_role] || ""}`}>
              ...
              {/* Bio & Experience */}
              {(selected.bio || selected.experience || selected.message) && (
              ...
              {/* Social & Links */}
              {(selected.facebook_url || selected.instagram_url || selected.youtube_url || selected.website_url || selected.portfolio_url) && (
                <div className="space-y-2">
                  <h3 className="text-sm font-semibold text-muted-foreground uppercase tracking-wider">Links</h3>
                  <div className="flex flex-wrap gap-2">
                    {[
                      { url: selected.portfolio_url, label: "Portfolio" },
                      { url: selected.website_url, label: "Website" },
                      { url: selected.facebook_url, label: "Facebook" },
                      { url: selected.instagram_url, label: "Instagram" },
                      { url: selected.youtube_url, label: "YouTube" },
              ...
              {selected.status === "pending" && (
                <div className="flex gap-3 pt-2 border-t border-border/30">
                  ...
                  <Button variant="outline" className="flex-1 text-destructive border-destructive/30" onClick={() => handleAction("rejected")} disabled={actionLoading}>
