...
import { Headphones, BookOpen, Star, WifiOff, Zap, Smartphone, Lock } from "lucide-react"
import { useSiteSettings } from "@/hooks/useSiteSettings"

interface Props {
  open: boolean
  /** Called when the user clicks a store download link. Modal closes; content stays locked. */
  onDownloadClick: () => void
  type: "audiobook" | "ebook"
  bookTitle?: string
}

export function MobileAppPromptModal({ open, onDownloadClick, type, bookTitle }: Props) {
  ...
  return (
    <Dialog open={open} onOpenChange={() => { /* non-dismissible */ }}>
      <DialogContent
        className="max-w-[340px] p-0 gap-0 overflow-hidden border-border/50 [&>button]:hidden"
        onInteractOutside={(e) => e.preventDefault()}
        onEscapeKeyDown={(e) => e.preventDefault()}
      >
        {/* Lock indicator */}
        <div className="flex items-center gap-2 bg-amber-500/15 border-b border-amber-500/20 px-4 py-2">
          <Lock className="w-3.5 h-3.5 text-amber-400 flex-shrink-0" />
          <p className="text-xs text-amber-300 font-medium">
            {isAudio
              ? "সম্পূর্ণ অডিওবুক শুনতে অ্যাপ ডাউনলোড করুন"
              : "সম্পূর্ণ বই পড়তে অ্যাপ ডাউনলোড করুন"}
          </p>
        </div>
        ...
        <div className="p-5 space-y-4">
          ...
          {/* Download buttons — clicking any of them locks content & closes modal */}
          <div className="space-y-2 pt-1">
            {playStoreUrl ? (
              <Button className="w-full gap-2 bg-foreground text-background hover:bg-foreground/90 rounded-xl" asChild>
                <a href={playStoreUrl} target="_blank" rel="noopener noreferrer" onClick={onDownloadClick}>
            ...
            {appStoreUrl ? (
              <Button variant="outline" className="w-full gap-2 rounded-xl" asChild>
                <a href={appStoreUrl} target="_blank" rel="noopener noreferrer" onClick={onDownloadClick}>
                  <svg className="w-5 h-5 flex-shrink-0" viewBox="0 0 24 24" fill="currentColor">
