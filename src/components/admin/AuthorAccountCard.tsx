...
export function AuthorAccountCard({ authorId, authorName, userId, onLinkChanged }: AuthorAccountCardProps) {
  ...
  const fetchLinkedUser = async (uid: string) => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke("admin-create-creator", {
        body: { action: "get_linked_user", userId: uid },
      });
      if (error || data?.error) {
        console.error("Fetch linked user error:", error || data?.error);
      } else {
  ...
  const handleUnlink = async () => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke("admin-create-creator", {
        body: { action: "unlink_profile", profileTable: "authors", profileId: authorId },
      });
      if (error || data?.error) {
  ...
  const handleLinkUser = async (targetUserId: string, targetEmail: string | null) => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke("admin-create-creator", {
        body: { action: "link_existing", email: targetEmail, role: "writer", profileTable: "authors", profileId: authorId },
      });
      if (error || data?.error) {
  ...
  // Called when user clicks "Link" on a search result — shows confirmation if changing
  const initiateLink = (user: SearchResult) => {
    if (userId) {
      // Already linked — show change confirmation
      ...
      <div className="p-4 rounded-lg border border-border/40 bg-secondary/30 space-y-3">
        ...
        <div className="grid grid-cols-2 gap-2 text-xs text-muted-foreground pt-1 border-t border-border/30">
          ...
          <div>
            ...
            <span>{linkedUser?.created_at ? new Date(linkedUser.created_at).toLocaleDateString() : "—"}</span>
          </div>
        </div>

        <div className="flex gap-2 pt-1">
          <Button size="sm" variant="outline" className="flex-1 text-xs h-8" onClick={() => window.open(`/admin/users?user=${linkedUser?.user_id}`, "_blank")}>
      ...
      <AlertDialog open={unlinkOpen} onOpenChange={setUnlinkOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will remove the linked account from <strong>{authorName}</strong> and stop earnings tracking. The creator's role will be revoked and they will lose access to creator panels. Historical earnings will be preserved. Are you sure?
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
          ...
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="space-y-2">
              <span className="block">
                You are about to change the linked account for <strong>{authorName}</strong>.
              </span>
              <span className="block text-xs">
                <strong>Current:</strong> {linkedUser?.display_name} ({linkedUser?.email})
              </span>
              <span className="block text-xs">
                <strong>New:</strong> {changeLinkTarget?.display_name} ({changeLinkTarget?.email})
              </span>
              <span className="block text-xs text-muted-foreground mt-1">
                Historical earnings under the old account will be preserved. Future earnings will go to the new account. The old user's creator role may be revoked.
              </span>
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              onClick={() => changeLinkTarget && handleLinkUser(changeLinkTarget.user_id, changeLinkTarget.email)}
              disabled={linking}
            >
...
function UserSearchPanel({
  searchQuery, setSearchQuery, searching, onSearch, results, linking, onLink, onCancel,
}: {
  searchQuery: string;
        ...
        <div className="max-h-52 overflow-y-auto space-y-1">
          {results.map((u) => (
            <div key={u.user_id} className="flex items-center gap-2 p-2 rounded-md hover:bg-secondary/60 transition-colors">
              ...
              <Button
                size="sm"
                variant={u.existing_links?.some(l => l.type === "author") ? "destructive" : "outline"}
                className="h-6 text-[10px] px-2 flex-shrink-0"
                onClick={() => onLink(u)}
                disabled={linking}
              >
                {u.existing_links?.some(l => l.type === "author") ? "Linked!" : "Link"}
              </Button>
            </div>
          ))}
        </div>
