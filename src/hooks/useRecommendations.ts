...
export function useBecauseYouRead(format?: "ebook" | "audiobook" | "hardcopy", search?: string) {
  const { user } = useAuth();
  const query = trpc.books.becauseYouRead.useQuery({ format, search: search || undefined }, {
    enabled: !!user,
    staleTime: 5 * 60 * 1000,
  });
